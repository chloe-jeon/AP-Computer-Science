...
public class Picture extends SimplePicture 
{
	...
	public void grayScale() {
		...
		for (Pixel[] rowArray : pixels)
		{
			for (Pixel pixelObj : rowArray)
			{
				int grayValue = pixelObj.getRed() + pixelObj.getGreen() + pixelObj.getBlue();
				grayValue = (int) Math.round(grayValue / 3.0);
	...
	{
		// If Left Pixel is far enough away from Right 
		// Pixel (based on edgeDist), then set leftPixel
		// to Color black; else, set it to Color white
		...
		boolean[][] outline = new boolean[pixels.length][pixels[0].length];	//true = black
		...
		for (int i = 0; i < pixels.length - 1; i++)
		{
			...
			{
				if (pixels[i][j].colorDistance(pixels[i + 1][j].getColor()) > edgeDist)
				{
					outline[i][j] = true;
				}
			}
	...
	public void mirrorVertical()
	{
		...
		for (int i = 0; i < pixels.length; i++)
		{
			...
			{
				Color temp1 = pixels[i][j].getColor();
				Color temp2 = pixels[i][pixels[i].length - j - 1].getColor();
				pixels[i][j].setColor(temp2);
				pixels[i][pixels[i].length - j - 1].setColor(temp1);
			}
		}
		
	}
	
	public void contrast(int diff)
	{
		Pixel[][] pixels = this.getPixels2D();
		for (Pixel[] rowArray : pixels)
		{
			for (Pixel pixelObj : rowArray)
			{
				if (pixelObj.getRed() > 127)
					pixelObj.setRed(pixelObj.getRed() + diff);
				else
					pixelObj.setRed(pixelObj.getRed() - diff);
				
				if (pixelObj.getGreen() > 127)
					pixelObj.setGreen(pixelObj.getGreen() + diff);
				else
					pixelObj.setGreen(pixelObj.getGreen() - diff);
				
				if (pixelObj.getBlue() > 127)
					pixelObj.setBlue(pixelObj.getBlue() + diff);
				else
					pixelObj.setBlue(pixelObj.getBlue() - diff);
			}
		}
